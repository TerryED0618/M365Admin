...
Function Connect-M365Admin {
	<#
		...
		.SYNOPSIS
			Connects to M365 services (workloads) and on-premises Exchange mailbox management server.

		.DESCRIPTION
			Connects to M365 services (workloads):
		...
		.PARAMETER ConnectAzAccount
			...
			Microsoft service (workload) supported:
				Azure ActiveDirectory via Az.Resources module.
				All other Az.* modules and their supported resources and services (workloads).
		...
		.PARAMETER ConnectIPPSSession
			When specified connect to Information Protect using the ExchangeOnlineManagement module.
		...
		.PARAMETER ConnectMgGraph
			...
			Microsoft services (workloads) supported:
				...
				Information Protect via Microsoft.Graph.Identity.SignIns module.
				...
				All other Microsoft.Graph.* modules and their supported resources and services (workloads).
		...
		.EXAMPLE
			...
			Connect to all M365 services (workloads) that support certificate-based authentication.
			A certificate with the private key is stored in this workstation's certificate store location of either CurrentUser or LocalMachine.  A corresponding certificate with the public key is registered with the Azure AD application.
		...
		.EXAMPLE
			...
			Connect to all M365 services (workloads) that support BasicAuth or BasicAuth to ModernAuth.
		...
		.EXAMPLE
			...
			Connect to all M365 services (workloads) that support ModernAuth.
			A multi-factor authentication (MFA) prompt is expected when the current access token is expired.
		...
		.NOTES
			...
			2023-04-05 Terry E Dow - Removed ParameterSetName AppCertificateBasedAuthParameterSet from parameter -ConnectExchangeServer.  Replaced associated scriptblock in Switch command with: Throw 'Connecting as an application is not supported with Exchange Management Shell.'
	...
	Param (
		...
		# Microsoft 365 services (workloads) connection parameters:
	...
	Begin {
		...
		# The following Requires are not required.  Documenting latest external module version this function has been aligned with.
		#
		...
		# #Requires -Modules @{ ModuleName='Az.Accounts'; ModuleVersion='2.12' } # 2.12.1 (Connect-AzAccount) https://learn.microsoft.com/en-us/powershell/azure/release-notes-azureps https://learn.microsoft.com/en-us/powershell/module/az.accounts/
		...
		# #Requires -Modules @{ ModuleName='ExchangeOnlineManagement'; ModuleVersion='3.1' } # 3.1.0 (Connect-ExchangeOnline or Connect-IPPSSession) https://learn.microsoft.com/en-us/powershell/exchange/exchange-online-powershell https://learn.microsoft.com/en-us/powershell/exchange/scc-powershell
		...
		# #Requires -Modules @{ ModuleName='Microsoft.Graph.Authentication'; ModuleVersion='1.25' } # 1.25.0 (Connect-MgGraph) https://learn.microsoft.com/en-us/powershell/microsoftgraph/authentication-commands
		# #Requires -Modules @{ ModuleName='Microsoft.Graph.Identity.DirectoryManagement'; ModuleVersion='1.25' } # 1.25.0 (Connect-MgGraph) https://learn.microsoft.com/en-us/powershell/module/microsoft.graph.identity.directorymanagement/
		# #Requires -Modules @{ ModuleName='MicrosoftTeams'; ModuleVersion='5.1' } # 5.1.0 (Connect-MicrosoftTeams) https://learn.microsoft.com/en-us/microsoftteams/teams-powershell-release-notes
		# #Requires -Modules @{ ModuleName='Microsoft.Online.SharePoint.PowerShell'; ModuleVersion='16.0' } # 16.0.23508.12000 (Connect-SPOService) https://learn.microsoft.com/en-us/powershell/module/sharepoint-online/
		...
		Switch ( $PsCmdlet.ParameterSetName ) {
			...
			'AppCertificateBasedAuthParameterSet' {
				...
				If ( $ConnectExchangeServer ) {
					Throw 'Connecting as an application is not supported with Exchange Management Shell.'
				}
				#endregion
			...
			{ $PSItem -In ( 'UserBasicAuthWithCredentialParameterSet', 'UserBasicAuthWithUserNamePasswordFileParameterSet' ) } {
				...
				If ( $AdminCredentialPasswordFilePath ) {
					...
					$adminCredentialPassword = Get-Content -Path $AdminCredentialPasswordFilePath | ConvertTo-SecureString
